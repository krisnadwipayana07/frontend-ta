...
import { headCells } from "../../utils/DataTabelDummy2";
...
export default function LaporanPembatalan() {
  ...
  const [dataPembatalan, setDataPembatalan] = React.useState([]);
  ...
  const endPoint = "/v1/laporan/pembatalan";
  const endPointDownload = "/v1/laporan/pembatalan/download";
    ...
    axios
      ...
      .then((res) => {
        if (method === 0) {
          ...
          setDataPembatalan(res.data.data.list);
        } else {
          setDataPembatalan([...dataPembatalan, ...res.data.data.list]);
        }
  ...
  const handleChangePage = (event, newPage) => {
    ...
    setPage(newPage);
    console.log(
      dataPembatalan.length - rowsPerPage,
      "<",
      rowsPerPage * (page + 1)
    );
    if (dataPembatalan.length - rowsPerPage <= rowsPerPage * (page + 1)) {
      getData(dataPembatalan.length, 1);
    }
  };
    ...
    if (
      ...
      axios
        ...
        .then((res) => {
          ...
          FileDownload(
            res.data,
            `Laporan Pembatalan_${time}.${res.config.params.fileType}`
          );
          setIsLoading(false);
        });
    }
