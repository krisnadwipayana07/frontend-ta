...
export default function ImplementasiQrGelang() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .get(baseApi + `/v1/band?saleaddress=${values.saleAddress}`)
      .then((res) => {
        localStorage.setItem("saleAddress", res.data.data.saleAddress);
        navigate("/qr-gelang");
        console.log(res.data.data.saleAddress);
      })
      .catch((err) => {
        setOpen(true);
